...
export default async function page() {
  ...
  const [userProjects, authoriz] = await Promise.all([databaseDrizzle.query.usersProjects.findMany({
  ...
  getAuthorizationWithUser(session.user.id),
  ])



  const userOwnProject = userProjects.filter(u => u.project.ownerId === session.user.id).length
  return (
    <AuthorizationProvider value={authoriz}>
